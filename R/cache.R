...
#' @noRd
cache_tif <- function(source, cache = TRUE, hull = .global[["cache"]][["hull"]]) {
  
  #TODO get source, output SpatRaster
  
  source <- list(
    list(
      tbl = "normal_composite",
  ...
  prof <- .globals$sessprof$get()
  
  # Translate hangs over 10k bands.
  sf::gdal_utils("translate", 
                 source = "PG:host=%s port=%s dbname=%s user=%s password=%s schema=public table='%s' column=rast mode=2 where='rid IN (%s)'" |>
                   sprintf(prof$host, prof$port, prof$dbname, prof$user, prof$password, rastertbl, paste0(rid, collapse = ",")), 
                 destination = "%s.tif" |> sprintf(rastertbl), 
                 options = c("-of", "GTiff", "-ot", "Float32", "-co", "COMPRESS=LZW"), 
                 quiet = TRUE)
  
  sf::gdal_utils("warp", 
                 source = c("normal_composite_1_nontiled.tif", "normal_composite_2_nontiled.tif"), 
                 destination = "combined_non_tiled.tif", 
                 options = c("-of", "GTiff", "-ot", "Float32", "-co", "COMPRESS=LZW"), 
                 quiet = TRUE)
