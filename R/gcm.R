...
#' @template cache
#' @template run_nm
#' @details An input list, with two nodes, `tbl` and `layers`, suitable for `extract_rast_tbl`.
...
input_gcms <- function(
  ...
  if (FALSE) {dbname <- var_nm <- mod <- scenario <- period <- run <- NULL}
...
input_gcm_hist <- function(
  ...
  #Remove NSE CRAN check warnings
  if (FALSE) {dbname <- var_nm <- mod <- var <- month <- run <- year <- NULL}
...
input_gcm_ssp <- function(
  ...
  #Remove NSE CRAN check warnings
  if (FALSE){ dbname <- var_nm <- full_nm <- scenario <- period <- run <- NULL}
